...
export async function GET(
	...
	console.log("DEBUG: accessToken from cookies:", accessToken);
	...
	if (!res.ok) {
		console.error("ERROR: Failed to fetch file from FastAPI", res.status, await res.text());
	...
	if (!url || typeof url !== "string") {
		...
		return NextResponse.json({ error: "Presigned URL missing" }, { status: 500 });
	}
	...
	if (typeof url !== "string" && url !== undefined && url !== null) {
		if (typeof (url as { toString?: () => string }).toString === "function") {
			...
			console.error("ERROR: Presigned URL is not a string and has no toString method", url);
			return NextResponse.json({ error: "Invalid presigned URL type" }, { status: 500 });
		}
	}
	...
	if (!fileRes.ok || !fileRes.body) {
		console.error("ERROR: Failed to fetch file data from storage", fileRes.status, await fileRes.text());
		return NextResponse.json({ error: "Failed to fetch file data" }, { status: 500 });
	}
