...
import { convertToBase64, compressImage } from "@/lib/utils";
...
export default function AddFriendForm({
  ...
  const onSubmit = async (data: AddFriendFormValues) => {
    ...
    try {
      ...
      if (data.file) {
        // Compress file if larger than 2MB before converting to base64
        let fileToProcess = data.file;
        const originalSizeMB = fileToProcess.size / (1024 * 1024);

        if (fileToProcess.size > 2 * 1024 * 1024) {
          toast.info(`Compressing image (${originalSizeMB.toFixed(2)}MB)...`);
          try {
            fileToProcess = await compressImage(fileToProcess, 2);
            ...
            toast.warning("Could not compress image, using original file");
          }
        }

        // Convert uploaded file to base64
        base64Result = await new Promise<string>((resolve) => {
          convertToBase64(fileToProcess, (result) => {
            const base64String =
